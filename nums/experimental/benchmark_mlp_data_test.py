...
def one_step_fit_np(app, X, y, W_in_1, W_1_2, W_2_out):
    ...
    y_predict = np_sigmoid(app, S_2 @ W_2_out)
...
def benchmark_mlp(num_gpus, N_list, system_class_list, d=1000, optimizer=True, dtype=np.float32):
    ...
    for N in N_list:
            ...
            if True:
                if system_class in ["Cupy", "Numpy"]:
                    ...
                    if optimizer:
                        ...
                        y = feedforward_opt(app, X, W_in_1, W_1_2, W_2_out, num_gpus)
                    else:
        ...
        del (y_tmp, y, app, X_cp, y_cp, 
             W_in_1_cp, W_1_2_cp, W_2_out_cp)
